...
const mirinteaDialogues = {
    ...
    idle: [
        ...
        "カードの音、今日なんか特別に感じる…{name}のせい？",
        "今の配置…天才じゃん{name}…。ちょっと惚れた。",
        "{name}の手札の動き、本気出してきたって分かる…ぞくってした♡",
        "あ、いまの静寂…好き。二人だけのゲーム空気って感じ…。",
        "カード滑る音…ねぇ聞いて？可愛すぎて胸キュンしたんだけど。",
        "その置き方ずるいよ…みりんてゃの心臓にクリティカル入った…",
        "{name}が真剣モードになる瞬間、ずっと見てたい…。",
        "ふふ…いいね今の形。まるで二人の相性みたい♡",
        "ねぇ、みりんてゃのカード触る？ラッキー起きるかも…♡",
        "今の沈黙…恋落ちの前触れかと思った…びっくりした…♡",
        "なんか今日の{name}…やさしい。手つきで分かる…。",
        "{name}のターン待ってる間がいちばん長く感じる。ドキドキしちゃう…。",
        "いま、流れめっちゃ綺麗…こういうの運命って言うんだよ？",
        "カードが揃う音まで可愛いってどういうこと…{name}魔法使った？",
        "ねぇ…楽しい。ゲームなのに…なんでこんなに胸あったかいの…？"
    ],
    shuffle: [
        "にゃっ…動かせるカードもうないみたい…",
        "{name}、もう1回シャッフルしよっか…？",
...
// Helper: Check if any card in the list can be moved to foundation or tableau
// カードの束の中に、有効な手があるかチェックする検品係（デバッグ付き）
function hasUsefulCard(cards, player) {
    const foundations = gameState[player].foundations;
    const tableau = gameState[player].tableau;
    
    console.log(`[検品開始] ${player}のカードをチェック中... 枚数:${cards.length}`);

    for (const card of cards) {
        // 組札に置ける？
        for (let i = 0; i < 4; i++) {
            if (canMoveToFoundation(foundations[i], card)) {
                console.log(`[発見!] ${getCardFileName(card)} は組札(Foundation-${i})に置けるよ！`);
                return true;
            }
        }
        // 場札に置ける？
        for (let col = 0; col < 7; col++) {
            if (canMoveToTableau(tableau[col], card)) {
                console.log(`[発見!] ${getCardFileName(card)} は場札(Tableau-${col})に置けるよ！`);
                return true;
            }
        }
    }
    
    console.log(`[残念] ${player}のカードの中には、今すぐ使えるカードがなかったよ...`);
    return false;
}
...
function mirinteaAI() {
    ...
    // ★弱体化1：連続行動を制限！
    // 基本は1回だけ。30%の確率で「調子いいからもう1個動かしちゃお」ってなる（最大2回）
    // 前はここが「5」だったから強すぎたんだよねw
    const MAX_ACTIONS_PER_TURN = Math.random() < 0.3 ? 2 : 1;

    while (actionsTaken < MAX_ACTIONS_PER_TURN) {
        let movedThisLoop = false;

        // 優先度1: 【勝利確定】組札（右上のゴール）へ移動
        // ★弱体化2：20%の確率で見逃す（気づかないふり）
        if (Math.random() < 0.8) {
            // 1-1. Waste -> Foundation
            if (gameState.mirintea.waste.length > 0) {
                ...
                if (movedThisLoop) { actionsTaken++; continue; }
            }

            // 1-2. Tableau -> Foundation
            if (!movedThisLoop) { // Wasteで動いてない場合のみ
                for (let col = 0; col < 7; col++) {
                    const pile = gameState.mirintea.tableau[col];
                    if (pile.length > 0) {
                        ...
                        for (let i = 0; i < 4; i++) {
                            if (canMoveToFoundation(gameState.mirintea.foundations[i], card)) {
                                gameState.mirintea.foundations[i].push(pile.pop());
                                // めくれたカードを表にする
                                if (pile.length > 0 && !pile[pile.length - 1].faceUp) {
                                    pile[pile.length - 1].faceUp = true;
                                }
        ...
        // 優先度2: 【場を整える】場札同士の移動
        // ★弱体化3：ここは頭使うから50%の確率でサボる（考えるのをやめる）
        if (Math.random() < 0.5) {
            for (let fromCol = 0; fromCol < 7; fromCol++) {
                ...
                // 移動する意味があるかチェック
                const isKingOnEmpty = firstFaceUpIndex === 0 && cardToMove.rank === 'K';
                if (isKingOnEmpty) continue;
        ...
        // 優先度3: 【手札を使う】Waste -> Tableau
        // ★弱体化4：ここは70%くらいの確率で気づく
        if (Math.random() < 0.7) {
            if (gameState.mirintea.waste.length > 0) {
                const card = gameState.mirintea.waste[gameState.mirintea.waste.length - 1];
                for (let toCol = 0; toCol < 7; toCol++) {
        ...
        // 優先度4: 【新規開拓】山札をめくる
        // ここは他にする事がなければ100%やる（そうしないとゲーム止まっちゃうからね）
        if (!movedThisLoop && gameState.mirintea.stock.length > 0) {
